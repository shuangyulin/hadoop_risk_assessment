...
<script>
	export default {
		data() {
			return {
				...
				rules: {
					...
					zhuanyuanxingming: [
						{ required: true, message: '专员姓名不能为空', trigger: 'blur' },
					],
					xingbie: [
					],
					lianxidianhua: [
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`fengkongzhuanyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`fengkongzhuanyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
