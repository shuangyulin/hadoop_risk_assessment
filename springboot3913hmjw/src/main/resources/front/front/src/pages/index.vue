	...
	<div class="main-containers">
		<div class="body-containers">
			<div class="top-container">
				<!-- info -->
				<div class="top_title">
					<span @click="goMenu('/index/home')">基于Hadoop的信贷风险评估的数据可视化分析与预测系统的设计与实现</span>
				</div>
		...
		async getSession() {
			await this.$http.get(`${localStorage.getItem('UserTableName')}/session`, {emulateJSON: true}).then(async res => {
				if (res.data.code == 0) {
					...
					if(localStorage.getItem('UserTableName')== 'fengkongzhuanyuan') {
						localStorage.setItem('username', res.data.data.zhuanyuanzhanghao);
					}
